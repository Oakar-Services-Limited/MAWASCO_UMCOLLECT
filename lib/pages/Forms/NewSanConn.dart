...
class NewSanConn extends StatefulWidget {
  const NewSanConn({
    super.key,
  });

  @override
  State<NewSanConn> createState() => _NewSanConnState();
}

class _NewSanConnState extends State<NewSanConn> {
  ...
  Future<void> uploadPhoto() async {
    final pickedFile = await imagePicker.pickImage(
      source: ImageSource.gallery, // Open the gallery to select a photo
    );
  ...
  Future<void> prefillForm(data) async {
    ...
    setState(() {
      ...
      waterSource = data[0]["WaterSource"]?.toString() ?? "";
  ...
  try {
    ...
    if (editing == 'true') {
      response = await http.put(
        Uri.parse("${getUrl()}sanitation/$newsanconID"),
      ...
      response = await http.post(
        Uri.parse("${getUrl()}sanitation/create"),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
        },
        body: jsonEncode(<String, dynamic>{
