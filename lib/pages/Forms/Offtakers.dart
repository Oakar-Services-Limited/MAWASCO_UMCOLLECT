...
class Offtakes extends StatefulWidget {
  const Offtakes({
    super.key,
  });

  @override
  State<Offtakes> createState() => _OfftakesState();
}

class _OfftakesState extends State<Offtakes> {
  ...
  prefillForm(data) async {
    ...
    setState(() {
      offtakersID = data[0]["ID"]?.toString() ?? "";
      ...
      accountname = data[0]["AccountName"]?.toString() ?? "";
      accountnum = data[0]["AccountNumber"]?.toString() ?? "";
  ...
  try {
    ...
    if (editing == 'true') {
      response = await http.put(
        Uri.parse("${getUrl()}offtakes/$offtakersID"),
      ...
      response = await http.post(
        Uri.parse("${getUrl()}offtakes/create"),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
        },
        body: jsonEncode(<String, dynamic>{
