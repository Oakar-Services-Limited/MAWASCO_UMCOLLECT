...
class ConnectionChambers extends StatefulWidget {
  const ConnectionChambers({
    super.key,
  });

  @override
  State<ConnectionChambers> createState() => _ConnectionChambersState();
}

class _ConnectionChambersState extends State<ConnectionChambers> {
  ...
  prefillForm(data) async {
    ...
    setState(() {
      connChambersID = data[0]["ID"]?.toString() ?? '';
      name = data[0]["Name"]?.toString() ?? '';
      zone = data[0]["Zone"]?.toString() ?? '';
      subzone = data[0]["Subzone"]?.toString() ?? '';
      chambersize = data[0]["ChamberSize"]?.toString() ?? '';
      year = data[0]["YearOfInstallation"]?.toString() ?? '';
      noOfConnections = data[0]["NoOfConnections"]?.toString() ?? '';
      manhole = data[0]["ManholeConnectedTo"]?.toString() ?? '';
  ...
  try {
    ...
    if (editing == 'true') {
      response = await http.put(
        Uri.parse("${getUrl()}connectionchamber/$connChambersID"),
      ...
      response = await http.post(
        Uri.parse("${getUrl()}connectionchamber/create"),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
        },
        body: jsonEncode(<String, dynamic>{
