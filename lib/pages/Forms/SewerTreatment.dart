...
class SewerTreatment extends StatefulWidget {
  const SewerTreatment({
    super.key,
  });

  @override
  State<SewerTreatment> createState() => _SewerTreatmentState();
}

class _SewerTreatmentState extends State<SewerTreatment> {
  ...
  try {
    ...
    if (editing == 'true') {
      ...
      response = await http.post(
        Uri.parse("${getUrl()}sr/sewertreatment"),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
        },
        body: jsonEncode(requestBody),
